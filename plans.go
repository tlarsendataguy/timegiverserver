...
func East12(c *Calculator) []Step {
	return []Step{
		...
		//Meals
		LightBreakfast{at: c.departureLess1At(c.breakfast())},
		LightLunch{at: c.departureLess1At(c.lunch())},
		LightDinner{at: c.departureLess1At(c.dinner())},
		HeavyBreakfast{at: c.arrivalAt(c.breakfast())},
		HeavyLunch{at: c.arrivalAt(c.lunch())},
		HeavyDinner{at: c.arrivalAt(c.dinner())},
...
func East34(c *Calculator) []Step {
	...
	return []Step{
		...
		//Meals
		HeavyBreakfast{at: c.departureLess2At(c.breakfast())},
		HeavyLunch{at: c.departureLess2At(c.lunch())},
		HeavyDinner{at: c.departureLess2At(c.dinner())},
		LightBreakfast{at: c.departureLess1At(c.breakfast())},
		LightLunch{at: c.departureLess1At(c.lunch())},
		LightDinner{at: c.departureLess1At(pre1Dinner)},
		HeavyBreakfast{at: c.arrivalAt(c.breakfast())},
		HeavyLunch{at: c.arrivalAt(c.lunch())},
		HeavyDinner{at: c.arrivalAt(c.dinner())},
...
func East56(c *Calculator) []Step {
	...
	return []Step{
		...
		//Meals
		HeavyBreakfast{at: c.departureLess4At(c.breakfast())},
		HeavyLunch{at: c.departureLess4At(c.lunch())},
		HeavyDinner{at: c.departureLess4At(c.dinner())},
		LightBreakfast{at: c.departureLess3At(c.breakfast())},
		LightLunch{at: c.departureLess3At(c.lunch())},
		LightDinner{at: c.departureLess3At(c.dinner())},
...
func East78(c *Calculator) []Step {
	...
	return []Step{
		...
		LightBreakfast{at: c.departureLess1At(c.breakfast())},
		LightLunch{at: c.departureLess1At(c.lunch())},
		LightDinnerOptional{at: c.departureLess1At(pre1Dinner)},
		HeavyBreakfast{at: c.arrivalAt(c.breakfast())},
		HeavyLunch{at: c.arrivalAt(c.lunch())},
		HeavyDinner{at: c.arrivalAt(c.dinner())},
		NoSnack{start: c.arrivalAt(c.dinner() + oneHour), end: c.arrivalAt(c.sleep())},
...
func East910(c *Calculator) []Step {
	...
	return []Step{
		...
		HeavyBreakfast{at: c.arrivalAt(c.breakfast())},
		HeavyLunch{at: c.arrivalAt(c.lunch())},
		HeavyDinner{at: c.arrivalAt(arrivalDinner)},
...
func West56(c *Calculator) []Step {
	...
	return []Step{
		...
		HeavyLunch{at: c.departureLess2At(c.lunch())},
		HeavyDinner{at: c.departureLess2At(c.dinner())},
		LightBreakfast{at: c.departureLess1At(c.breakfast() + twoHours)},
		LightLunch{at: c.departureLess1At(c.lunch() + twoHours)},
		LightDinner{at: c.departureLess1At(c.dinner() + twoHours)},
		HeavyBreakfast{at: c.arrivalAt(c.breakfast())},
		HeavyLunch{at: c.arrivalAt(c.lunch())},
		HeavyDinner{at: c.arrivalAt(c.dinner())},
...
func West78(c *Calculator) []Step {
	...
	return []Step{
		...
		//Meals
		HeavyBreakfast{at: c.departureLess3At(c.breakfast())},
		HeavyLunch{at: c.departureLess3At(c.lunch())},
		HeavyDinner{at: c.departureLess3At(c.dinner())},
		LightBreakfast{at: c.departureLess2At(c.breakfast())},
		LightLunch{at: c.departureLess2At(c.lunch())},
		LightDinner{at: c.departureLess2At(c.dinner())},
		NoSnack{start: c.departureLess2At(c.dinner() + oneHour), end: c.departureLess2At(c.sleep())},
		HeavyBreakfast{at: c.departureLess1At(c.breakfast())},
		HeavyLunch{at: c.departureLess1At(c.lunch())},
		HeavyDinner{at: c.departureLess1At(c.dinner())},
		LightBreakfast{at: c.departureAt(departureBreakfast)},
		HeavyBreakfast{at: c.arrivalAt(c.breakfast())},
		HeavyLunch{at: c.arrivalAt(c.lunch())},
		HeavyDinner{at: c.arrivalAt(c.dinner())},
...
func West910(c *Calculator) []Step {
	...
	return []Step{
		...
		HeavyDinner{at: c.departureLess1At(c.dinner())},
		LightBreakfast{at: c.departureAt(departureBreakfast)},
		LightLunch{at: c.departureAt(departureLunch)},
		HeavyBreakfast{at: c.arrivalAt(c.breakfast())},
		HeavyLunch{at: c.arrivalAt(c.lunch())},
		HeavyDinner{at: c.arrivalAt(c.dinner())},
